...
> Docker镜像：6-8MB（Scratch）、10-12MB（Alpine）
...
**小访问量优化**（2026-01-10）：
- ✅ Redis 连接池优化：10→3个连接
- ✅ 速率限制优化：100次/分钟→60次/小时
- ✅ 移除健康检查（简化部署）
- ✅ 预设中国时区
- ✅ Docker 简化（Scratch 版作为最终版）
- ✅ 文档合并清理（4个MD→1个）
...
- ✅ 修复前端 API 地址硬编码
  ...
  - 新代码：支持环境变量 `VITE_API_URL`
  - 默认值：`http://localhost:7066`
